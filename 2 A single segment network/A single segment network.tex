...
While \lstbash[emph={lo}]{tcpdump -i lo &}\footnote{In old linux use \lstbash[emph={your-host}]{tcpdump host your-host}} is running in one command window, run \lstbash{ping 127.0.0.1} from another command window.
...
You can run \lstbash{wireshark} to capture network packet by \textbf{start capture} in GNS3 link menu.
